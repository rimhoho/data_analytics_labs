...
lm_matrix_trees %>% arrange(desc(.resid)) %>% head()

lm_matrix_trees$.resid_abs <- abs(lm_matrix_trees$.resid)
lm_matrix_trees %>% arrange(desc(.resid_abs)) %>% head()
...
ggplot(data=trees, aes(x=Girth, y=Height)) + geom_point() +
  geom_smooth(method = 'lm') + geom_point(data=mytree, aes(y=.fitted), size = 3, color = 'red') +
  labs(title='Tree height by girth')
