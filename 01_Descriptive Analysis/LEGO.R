...
by_set <- merged_df %>% filter(sets_name %in% target_set) %>% group_by(year, sets_name) %>% 
  summarize(quantity_of_sets = sum(as.numeric(quantity))) %>% arrange(desc(quantity_of_sets))
glimpse(by_set)
summary(by_set)
...
target_color <- c('Black', 'Light Bluish Grey', 'White', 'Dark Bluish Gray', 'Red', 'Reddish Brown', 'Tan', 'Yellow')
by_set <- merged_df %>% filter(color_name %in% target_color) %>% group_by(year, color_name) %>% 
  summarize(quantity_of_sets = sum(as.numeric(quantity))) %>% arrange(desc(quantity_of_sets))
glimpse(by_set)
summary(by_set)

by_colorname_gr <- ggplot(by_set, aes(x = year, y = quantity_of_sets, color = color_name)) + 
  geom_point(size = 4, alpha = 0.6) +
  facet_grid(.~color_name) +
  labs(title = "Changes in colors of sets") 
by_colorname_gr + theme_economist(horizontal=FALSE) + scale_colour_economist()
